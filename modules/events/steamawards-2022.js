...
module.exports = async function(steamClient, RequestCommunity, RequestStore, SessionID, options, callback){
    if(!await EnsureWeAreDone()){
        ...
        await new Promise(resolve => setTimeout(resolve, 1000 * 60 * 5)); // need 5 min to make a review
    ...
    function HaveGotPlayAGame() {
        return new Promise(function (resolve) {
            RequestStore.get('https://store.steampowered.com/steamawards/nominations', function (error, response, body) {
                var $ = cheerio.load(body);
                setTimeout(function(){
                    resolve($(".badge_preview.level_3.current").length > 0);
                }, 2000);
            });
        });
    }
