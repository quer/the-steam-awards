...
module.exports = async function(steamClient, RequestCommunity, RequestStore, SessionID, options, callback)
{
    ...
    try {
        ...
        if(startLevel != 10){
            console.log("account was level '" + startLevel +"' will try to get rest")
            ...
            while (!accountDone) {
                ...
                if(badgeLevel != 10){
                    if(loops > 10){
                        accountDone = true;
                        console.logError("After 10 full Run's. it was not able to get the bagde to max level. Steam might be down. try agirn later.");

                    }else{
                        console.log("account was not able to get max level, it will rerun the module")
                        loops += 1;
                    }
                }else {
                    accountDone = true;
                    console.log("account now have max level");
                }
            }
        } else {
            console.log("account allready have max level")
        }

    } catch (error) {
        console.logError("Somefing happende. You have to rerun the module. if the same error happen offen, create a issue on the github page" + error);
        ...
        return new Promise(function (resolve) {  
            RequestStore.get({ url: link }, async function (linkError, linkResponse, linkBody){
                try {
                    ...
                    console.log("Failed to obtain badge!. Just give it time, i will get this at some point");
        ...
        return new Promise(function (resolve, reject) {  
            RequestStore.get('https://store.steampowered.com/sale/clorthax_quest', function (error, response, html) {
                ...
                if(authwgtoken == "" || !authwgtoken.authwgtoken){
                    console.log("error no authwgtoken");
                    reject();
                }else{
