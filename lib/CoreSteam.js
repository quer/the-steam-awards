    ...
    this.Run = function () {
        return new Promise(async function (resolve, reject) {
            ...
            steamClient.on('connected', function() {
                ...
                }else if(this.auth[settings.AuthFieldNameTwoFactorCode] && this.auth[settings.AuthFieldNameTwoFactorCode] != null){
                    LoginObj.two_factor_code = this.auth[settings.AuthFieldNameTwoFactorCode];
                }else{
            ...
            steamClient.on('logOnResponse', async function onSteamLogOn(logonResp) {
                ...
                if (logonResp.eresult == Steam.EResult.OK) {
                    ...
                    var { _requestCommunity, _requestStore, _requestCheckoutStore, sessionID, newCookie } = {_requestCommunity: {}, _requestStore: {}, _requestCheckoutStore: {}, sessionID: "", newCookie: []}//await this.websession(steamWebLogOn, steamClient, steamUser);
            ...
            var { _requestCommunity, _requestStore, _requestCheckoutStore, sessionID, newCookie } = await require('./WebCookies')(this.auth, this)//await this.websession(steamWebLogOn, steamClient, steamUser);
            var shoudRunAccount = true;
            if(this.auth[settings.AuthFieldNameFamilyViewPIN]){
                try {
                    await this.SetSteamParental(_requestCommunity, sessionID, this.auth[settings.AuthFieldNameFamilyViewPIN]);
            ...
            if(shoudRunAccount){
                ...
                for (let i = 0; i < this.modules.length; i++) {
                    ...
                    try {
                        await this.runModule(module.module, {}, _requestCommunity, _requestStore, sessionID, options);                            
                    } catch (error) {
                        this.logError("Module failed");
                        this.logError(error);
                    }
    ...
    this.websession = function(steamWebLogOn) {	
        return new Promise(function (resolve, reject) {
            ...
            steamWebLogOn.webLogOn(function(sessionID, newCookie) {
                ...
                resolve({ _requestCommunity, _requestStore, _requestCheckoutStore, sessionID, newCookie });
            });
        }.bind(this));
    }*/
